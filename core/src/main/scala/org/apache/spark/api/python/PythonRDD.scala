...
/**
 * Form an RDD[(Array[Byte], Array[Byte])] from key-value pairs returned from Python. This is used
 * by PySpark's shuffle operations.
 */
...
private[spark] object PythonRDD extends Logging {
   ...
   * (effectively a collect()), but allows you to run on a certain subset of partitions, or to
   * enable local execution.
   *
   * @return
   *   3-tuple (as a Java array) with the port number of a local socket which serves the data
   *   collected from this job, the secret for authentication, and a socket auth server object
   *   that can be used to join the JVM serving thread in Python.
   */
  def runJob(
      sc: SparkContext,
    ...
    val flattenedPartition: UnrolledPartition =
      Array.concat(allPartitions.toImmutableArraySeq: _*)
    serveIterator(
  ...
  /**
   * A helper function to collect an RDD as an iterator, then serve it via socket. This method is
   * similar with `PythonRDD.collectAndServe`, but user can specify job group id, job description,
   * and interruptOnCancel option.
   */
  ...
  /**
   * A helper function to create a local RDD iterator and serve it via socket. Partitions are are
   * collected as separate jobs, by order of index. Partition data is first requested by a
   * non-zero integer to start a collection job. The response is prefaced by an integer with 1
   * meaning partition data will be served, 0 meaning the local iterator has been consumed, and -1
   * meaning an error occurred during collection. This function is used by
   ...
   * written to the stream. Returns false if no data was written as the iterator has been
   * exhausted.
   */
  ...
  /**
   * Create an RDD from a path using [[org.apache.hadoop.mapred.SequenceFileInputFormat]], key and
   * value class. A key and/or value converter class can optionally be passed in (see
  ...
  /**
   * Create an RDD from a file path, using an arbitrary
   * [[org.apache.hadoop.mapreduce.InputFormat]], key and value class. A key and/or value
   * converter class can optionally be passed in (see [[org.apache.spark.api.python.Converter]])
   */
   ...
   * passed in from Python, using an arbitrary [[org.apache.hadoop.mapreduce.InputFormat]], key
   * and value class. A key and/or value converter class can optionally be passed in (see
   * [[org.apache.spark.api.python.Converter]])
   */
   ...
   * key and value class. A key and/or value converter class can optionally be passed in (see
   * [[org.apache.spark.api.python.Converter]])
   */
  ...
  /**
   * Create an RDD from a [[org.apache.hadoop.conf.Configuration]] converted from a map that is
   * passed in from Python, using an arbitrary [[org.apache.hadoop.mapred.InputFormat]], key and
   * value class A key and/or value converter class can optionally be passed in (see
   * [[org.apache.spark.api.python.Converter]])
   */
  def hadoopRDD[K, V, F <: InputFormat[K, V]](
    ...
    val conf = getMergedConf(confAsMap, sc.hadoopConfiguration())
    val rdd =
      hadoopRDDFromClassNames[K, V, F](sc, None, inputFormatClass, keyClass, valueClass, conf)
    val confBroadcasted = sc.sc.broadcast(new SerializableConfiguration(conf))
    val converted = convertRDD(
      rdd,
      keyConverterClass,
   ...
   *
   * Once a connection comes in, it tries to serialize all the data in `items` and send them into
   * this connection.
   *
  ...
  /**
   * Create a socket server and background thread to execute the writeFunc with the given
   * OutputStream.
   *
   * The socket server can only accept one connection, or close if no connection in 15 seconds.
   *
   * Once a connection comes in, it will execute the block of code and pass in the socket output
   * stream.
   *
   * The thread will terminate after the block of code is executed or any exceptions happen.
   *
   * @return
   *   3-tuple (as a Java array) with the port number of a local socket which serves the data
   *   collected from this job, the secret for authentication, and a socket auth server object
  ...
  private def inferKeyValueTypes[K, V, KK, VV](
      rdd: RDD[(K, V)],
      keyConverterClass: String = null,
  ...
  private def getKeyValueTypes[K, V](
      keyClass: String,
      valueClass: String): Option[(Class[K], Class[V])] = {
    for {
      k <- Option(keyClass)
      v <- Option(valueClass)
   ...
   */
  private def convertRDD[K, V](
      rdd: RDD[(K, V)],
      keyConverterClass: String,
      valueConverterClass: String,
      defaultConverter: Converter[Any, Any]): RDD[(Any, Any)] = {
    val (kc, vc) = getKeyValueConverters[K, V, Any, Any](
      keyConverterClass,
      valueConverterClass,
      defaultConverter)
  ...
  /**
   * Output a Python RDD of key-value pairs as a Hadoop SequenceFile using the Writable types we
   * convert from the RDD's key and value types. Note that keys and values can't be
   * [[org.apache.hadoop.io.Writable]] types already, since Writables are not Java `Serializable`
   * and we can't peek at them. The `path` can be on any Hadoop file system.
   */
   ...
   * `OutputFormat` in mapred package. Keys and values are converted to suitable output types
   * using either user specified converters or, if not specified,
   * [[org.apache.spark.api.python.JavaToWritableConverter]]. Post-conversion types `keyClass` and
   * `valueClass` are automatically inferred if not specified. The passed-in `confAsMap` is merged
   * with the default Hadoop conf associated with the SparkContext of this RDD.
   */
  def saveAsHadoopFile[F <: OutputFormat[_, _], C <: CompressionCodec](
    ...
    val converted =
      convertRDD(rdd, keyConverterClass, valueConverterClass, new JavaToWritableConverter)
   ...
   * `OutputFormat` in mapreduce package. Keys and values are converted to suitable output types
   * using either user specified converters or, if not specified,
   * [[org.apache.spark.api.python.JavaToWritableConverter]]. Post-conversion types `keyClass` and
   * `valueClass` are automatically inferred if not specified. The passed-in `confAsMap` is merged
   * with the default Hadoop conf associated with the SparkContext of this RDD.
   ...
   * converted from the passed-in `confAsMap`. The conf should set relevant output params ( e.g.,
   * output path, output format, etc), in the same way as it would be configured for a Hadoop
   * MapReduce job. Both old and new Hadoop OutputFormat APIs are supported (mapred vs.
   * mapreduce). Keys/values are converted for output using either user specified converters or,
   * by default, [[org.apache.spark.api.python.JavaToWritableConverter]].
   */
  def saveAsHadoopDataset(
    ...
    val converted = convertRDD(
      SerDeUtil.pythonToPairRDD(pyRDD, batchSerialized),
      keyConverterClass,
      valueConverterClass,
      new JavaToWritableConverter)
    if (useNewAPI) {
...
private class BytesToString
    extends org.apache.spark.api.java.function.Function[Array[Byte], String] {
  override def call(arr: Array[Byte]): String = new String(arr, StandardCharsets.UTF_8)
}

/**
 * Internal class that acts as an `AccumulatorV2` for Python accumulators. Inside, it collects a
 * list of pickled strings that we pass to Python through a socket.
 */
private[spark] class PythonAccumulatorV2(
  ...
  /**
   * We try to reuse a single Socket to transfer accumulator updates, as they are all added by the
   * DAGScheduler's single-threaded RpcEndpoint anyway.
   */
  ...
  private def openSocket(): Socket = synchronized {
    if (socket == null || socket.isClosed) {
      ...
      logInfo(
        log"Connected to AccumulatorServer at host: ${MDC(HOST, serverHost)}" +
    ...
    if (!diskBlockManager.containsBlock(blockId)) {
      Utils.tryOrIOException {
        ...
        Utils.tryWithSafeFinally {
          ...
          val blockStoreUpdater = blockManager.TempFileBasedBlockStoreUpdater(
            blockId,
 ...
 *
 * We might be serializing a really large object from python -- we don't want python to buffer the
 * whole thing in memory, nor can it write to a file, so we don't know the length in advance. So
 * python writes it in chunks, each chunk preceded by a length, till we get a "length" of -1 which
 * serves as EOF.
 *
...
private[spark] class EncryptedPythonBroadcastServer(
    ...
    extends SocketAuthServer[Unit]("broadcast-decrypt-server")
    with Logging {
...
/**
 * Helper for making RDD[Array[Byte]] from some python data, by reading the data from python over
 * a socket. This is used in preference to writing data to a file when encryption is enabled.
 */
