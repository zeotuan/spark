...
/**
 * An implementation of checkpointing that writes the RDD data to reliable storage. This allows
 * drivers to be restarted on failure with previously computed state.
 */
private[spark] class ReliableRDDCheckpointData[T: ClassTag](@transient private val rdd: RDD[T])
  ...
  /**
   * Return the directory to which this RDD was checkpointed. If the RDD is not checkpointed yet,
   * return None.
   */
  ...
  /**
   * Materialize this RDD and write its content to a reliable DFS. This is called immediately
   * after the first action invoked on this RDD has completed.
   */
