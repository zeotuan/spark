 ...
 */
private class ShuffleStatus(numPartitions: Int, numReducers: Int = -1) extends Logging {
  ...
  /**
   * MapStatus for each partition. The index of the array is the map partition id. Each value in
   * the array is the MapStatus for a partition, or null if the partition is not available. Even
   * though in theory a task may run multiple times (due to speculation, stage retries, etc.), in
   * practice the likelihood of a map output being available at multiple locations is so small
   * that we choose to ignore that case and store only a single location for each output.
   */
  // Exposed for testing
  ...
  /**
   * Counter tracking the number of partitions that have output. This is a performance
   * optimization to avoid having to count the number of non-null entries in the `mapStatuses`
   * array and should be equivalent to`mapStatuses.count(_ ne null)`.
   */
  ...
  /**
   * Mapping from a mapId to the mapIndex, this is required to reduce the searching overhead
   * within the function updateMapOutput(mapId, bmAddress).
   */
  ...
  /**
   * Remove the map output which was served by the specified block manager. This is a no-op if
   * there is no registered map output or if the registered output is from a different block
   * manager.
   */
  ...
  /**
   * Removes all shuffle outputs which satisfies the filter. Note that this will also remove
   * outputs which are served by an external shuffle server (if one exists).
   */
  ...
  /**
   * Number of shuffle partitions that have already been merge finalized when push-based is
   * enabled.
   */
  ...
  /**
   * Serializes the mapStatuses and mergeStatuses array into an efficient compressed format. See
  ...
  /**
   * Helper function which provides thread-safe access to the mapStatuses array. The function
...
private[spark] case class GetMapOutputStatuses(shuffleId: Int) extends MapOutputTrackerMessage
...
private[spark] case class GetMapAndMergeOutputMessage(shuffleId: Int, context: RpcCallContext)
    extends MapOutputTrackerMasterMessage
private[spark] case class GetShufflePushMergersMessage(shuffleId: Int, context: RpcCallContext)
    extends MapOutputTrackerMasterMessage
...
/**
 * Class that keeps track of the location of the map output of a stage. This is abstract because
 * the driver and executor have different versions of the MapOutputTracker. In principle the
 * driver- and executor-side classes don't need to share a common base class; the current shared
 * base class is maintained primarily for backwards-compatibility in order to avoid having to
...
private[spark] abstract class MapOutputTracker(conf: SparkConf) extends Logging {
  ...
  /**
   * The driver-side counter is incremented every time that a map output is lost. This value is
   * sent to executors as part of tasks, where executors compare the new epoch number to the
   * highest epoch number that they received in the past. If the new epoch number is higher then
   * executors will clear their local caches of map output statuses and will re-fetch (possibly
   * updated) statuses from the driver.
   */
  protected var epoch: Long = 0
  protected val epochLock = new AnyRef

  /**
   * Send a message to the trackerEndpoint and get its result within a default timeout, or throw a
  ...
  def getMapSizesByExecutorId(
      shuffleId: Int,
      reduceId: Int): Iterator[(BlockManagerId, collection.Seq[(BlockId, Long, Int)])] = {
  ...
  def getPushBasedShuffleMapSizesByExecutorId(
      shuffleId: Int,
      reduceId: Int): MapSizesByExecutorId = {
   ...
   *
   * @return
   *   A sequence of 2-item tuples, where the first item in the tuple is a BlockManagerId, and the
   *   second item is a sequence of (shuffle block id, shuffle block size, map index) tuples
   *   describing the shuffle blocks that are stored at that block manager. Note that zero-sized
   *   blocks are excluded in the result.
   */
  def getMapSizesByExecutorId(
      shuffleId: Int,
      startMapIndex: Int,
   ...
   *
   * @return
   *   A case class object which includes two attributes. The first attribute is a sequence of
   *   2-item tuples, where the first item in the tuple is a BlockManagerId, and the second item
   *   is a sequence of (shuffle block id, shuffle block size, map index) tuples tuples describing
   *   the shuffle blocks that are stored at that block manager. Note that zero-sized blocks are
   *   excluded in the result. The second attribute is a boolean flag, indicating whether batch
   *   fetch can be enabled.
   */
  def getPushBasedShuffleMapSizesByExecutorId(
      shuffleId: Int,
      startMapIndex: Int,
  ...
  /**
   * Called from executors upon fetch failure on an entire merged shuffle reduce partition. Such
   * failures can happen if the shuffle client fails to fetch the metadata for the given merged
   * shuffle partition. This method is to get the server URIs and output sizes for each shuffle
   * block that is merged in the specified merged shuffle block so fetch failure on a merged
   * shuffle block can fall back to fetching the unmerged blocks.
   *
   * @return
   *   A sequence of 2-item tuples, where the first item in the tuple is a BlockManagerId, and the
   *   second item is a sequence of (shuffle block ID, shuffle block size, map index) tuples
   ...
   * to get the server URIs and output sizes for each shuffle block that is merged in the
   * specified merged shuffle partition chunk so fetch failure on a merged shuffle block chunk can
   * fall back to fetching the unmerged blocks.
   *
   * chunkBitMap tracks the mapIds which are part of the current merged chunk, this way if there
   * is a fetch failure on the merged chunk, it can fallback to fetching the corresponding
   * original blocks part of this merged chunk.
   *
   * @return
   *   A sequence of 2-item tuples, where the first item in the tuple is a BlockManagerId, and the
   *   second item is a sequence of (shuffle block ID, shuffle block size, map index) tuples
   *   describing the shuffle blocks that are stored at that block manager.
   */
  def getMapSizesForMergeResult(shuffleId: Int, partitionId: Int, chunkBitmap: RoaringBitmap)
   ...
   * mergers available since very few executors got registered. This is on a best effort basis, if
   * there is not enough shuffle mergers available for this stage then an empty sequence would be
   * returned indicating the task to avoid shuffle push.
   * @param shuffleId
   */
...
private[spark] class MapOutputTrackerMaster(
   ...
   *   the reduceId.
   * @param bmAddress
   *   block manager address.
   * @param mapIndex
   *   the optional mapIndex which should be checked to see it was part of the merge result.
   */
   ...
   *
   * @param dep
   *   shuffle dependency object
   * @param partitionId
   *   map output partition that we want to read
   * @return
   *   a sequence of host names
   */
   ...
   *   id of the shuffle
   * @param reducerId
   *   id of the reduce task
   * @param numReducers
   *   total number of reducers in the shuffle
   * @param fractionThreshold
   *   fraction of total map output size that a location must have for it to be considered large.
   */
   ...
   *   the start map index
   * @param endMapIndex
   *   the end map index (exclusive)
   * @return
   *   a sequence of locations where task runs.
   */
  def getMapLocation(
      dep: ShuffleDependency[_, _, _],
      startMapIndex: Int,
...
private[spark] class MapOutputTrackerWorker(conf: SparkConf) extends MapOutputTracker(conf) {
  ...
  /**
   * [[shufflePushMergerLocations]] tracks shuffle push merger locations for the latest shuffle
   * execution
   *
   * Exposed for testing
   */
  ...
  /**
   * A [[KeyLock]] whose key is a shuffle id to ensure there is only one thread fetching the same
   * shuffle block.
   */
  ...
  /**
   * Get or fetch the array of MapStatuses and MergeStatuses if push based shuffle enabled for a
   * given shuffle ID. NOTE: clients MUST synchronize on this array when reading it, because on
   * the driver, we may be changing it in place.
   *
  ...
  /**
   * Called from executors to update the epoch number, potentially clearing old outputs because of
   * a fetch failure. Each executor task calls this with the latest epoch number on the driver at
...
private[spark] object MapOutputTracker extends Logging {
   ...
   * stored at that block manager. Note that empty blocks are filtered in the result.
   *
   * If push-based shuffle is enabled and an array of merge statuses is available, prioritize the
   * locations of the merged shuffle partitions over unmerged shuffle blocks.
   *
   * If any of the statuses is null (indicating a missing location due to a failed mapper), throws
   * a FetchFailedException.
   *
   * @param shuffleId
   *   Identifier for the shuffle
   * @param startPartition
   *   Start of map output partition ID range (included in range)
   * @param endPartition
   *   End of map output partition ID range (excluded from range)
   * @param mapStatuses
   *   List of map statuses, indexed by map partition index.
   * @param startMapIndex
   *   Start Map index.
   * @param endMapIndex
   *   End Map index.
   * @param mergeStatuses
   *   List of merge statuses, index by reduce ID.
   * @return
   *   A sequence of 2-item tuples, where the first item in the tuple is a BlockManagerId, and the
   *   second item is a sequence of (shuffle block id, shuffle block size, map index) tuples
   *   describing the shuffle blocks that are stored at that block manager.
  ...
  /**
   * Given a shuffle ID, a partition ID, an array of map statuses, and bitmap corresponding to
   * either a merged shuffle partition or a merged shuffle partition chunk, identify the metadata
   * about the shuffle partition blocks that are merged into the merged shuffle partition or
   * partition chunk represented by the bitmap.
   *
   * @param shuffleId
   *   Identifier for the shuffle
   * @param partitionId
   *   The partition ID of the MergeStatus for which we look for the metadata of the merged
   *   shuffle partition blocks
   * @param mapStatuses
   *   List of map statuses, indexed by map ID
   * @param tracker
   *   bitmap containing mapIndexes that belong to the merged block or merged block chunk.
   * @return
   *   A sequence of 2-item tuples, where the first item in the tuple is a BlockManagerId, and the
   *   second item is a sequence of (shuffle block ID, shuffle block size) tuples describing the
   *   shuffle blocks that are stored at that block manager.
   */
